...
interface PickupAvailabilityRow {
  id: string;
...
interface ReservationRow {
  ...
  note: string | null;
  status: 'pending' | 'confirmed' | 'cancelled' | 'completed';
  booking_type: 'exact' | 'flexible' | null;
  flexible_period: 'afternoon' | 'evening_early' | 'night' | 'tbd' | null;
  created_at: string;
}

interface ReservationWithAvailabilityRow extends ReservationRow {
  pickup_availability: { available_date: string } | null;
}

interface AvailabilityIdRow {
  availability_id: string;
}
...
function mapDbReservation(row: ReservationRow): Reservation {
  return {
    id: row.id,
...
export async function getAvailableDates(): Promise<PickupAvailability[]> {
  ...
  const typedAvails = avails as PickupAvailabilityRow[];
  const ids = typedAvails.map((a) => a.id);
  ...
  const typedReservs = (reservs ?? []) as AvailabilityIdRow[];
  const counts = new Map<string, number>();
  for (const r of typedReservs) {
    counts.set(r.availability_id, (counts.get(r.availability_id) || 0) + 1);
  }

  return typedAvails
    .map(
      (row): PickupAvailability => ({
        id: row.id,
...
export async function getAvailabilityById(id: string): Promise<PickupAvailability | null> {
  const sb = getSupabase();
  if (!sb) return null;
  const result = await sb.from('pickup_availability').select('*').eq('id', id).single();
  if (result.error) return null;
  const row = result.data as PickupAvailabilityRow;
...
export async function createReservation(input: {
  ...
  const sb = getSupabase();
  if (!sb) return null;
  const rpcResult = await sb.rpc('create_reservation_atomic', {
  ...
  if (rpcResult.error || !rpcResult.data) {
    console.error('reservation atomic insert error:', rpcResult.error);
    return null;
  }
...
export async function getAllReservations(dateFilter?: string): Promise<Reservation[]> {
  ...
  if (dateFilter) {
    const { data: avail } = await sb
      ...
      .single();
    if (!avail) return [];
    query = query.eq('availability_id', (avail as { id: string }).id);
  }
  ...
  return ((data ?? []) as ReservationWithAvailabilityRow[]).map((r) => ({
    ...mapDbReservation(r),
...
export async function updateReservationNote(id: string, note: string): Promise<boolean> {
  ...
  if (error) {
    console.error('reservation note update error:', error);
    return false;
  }
  return true;
}
...
export async function getReservationById(id: string): Promise<Reservation | null> {
  ...
  const row = result.data as ReservationWithAvailabilityRow;
  return { ...mapDbReservation(row), availableDate: row.pickup_availability?.available_date };
}
...
export async function updateReservationOrderNumber(
  id: string,
  orderNumber: string,
): Promise<boolean> {
  const sb = getSupabase();
...
export async function getConfirmedReservationsForCalendar(): Promise<Reservation[]> {
  ...
  const { data, error } = await sb
    ...
    .order('created_at', { ascending: false });
  if (error) {
    console.error('calendar reservations fetch error:', error);
    return [];
  }
  return ((data ?? []) as ReservationWithAvailabilityRow[]).map((r) => ({
    ...mapDbReservation(r),
