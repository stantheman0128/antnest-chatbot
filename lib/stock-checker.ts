import { getActiveProducts, upsertProduct, getConfig, setConfig } from "./data-service";
  ...
  const results = await Promise.allSettled(
    products.map(async (product) => {
      ...
      const updatedVariants = product.variants.map((v) => {
        const match = freshAvail.find(
          (f) => f.title === v.title || f.title === v.option1
        );
