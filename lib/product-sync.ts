...
export type SyncOutcome =
  | { ok: true; added: number; updated: number; unchanged: number; deactivated: number }
  | { ok: false; error: string; status: number };

/** Full product catalog sync from Cyberbiz. Callable directly (cron) or via the admin route. */
export async function runProductSync(): Promise<SyncOutcome> {
  const handles = await getProductHandles();
  if (handles.length === 0) {
    return { ok: false, error: 'No products found in sitemap', status: 502 };
  }
  ...
  return { ok: true, added, updated, unchanged, deactivated };
}
...
export async function syncSingleProduct(handle: string): Promise<SingleSyncOutcome> {
  const scraped = await scrapeProduct(handle);
  if (!scraped || !scraped.price) {
    return { ok: false, error: 'Product not found on Cyberbiz', status: 404 };
  }
  ...
  return { ok: true, product: scraped.name };
}
