...
interface ReservationWithPickupRow {
  line_user_id: string;
  order_number: string | null;
  status: string;
  pickup_availability: { available_date: string } | null;
}
...
export async function upsertLineUser(
  ...
  try {
    ...
    if (err?.code !== '42P01') console.error('upsertLineUser error:', e);
  }
}
...
export function logConversation(
  ...
  void sb
    ...
    .then(({ error }) => {
      if (error && error.code !== '42P01') console.error('logConversation error:', error);
    });
}
...
export async function getAllLineUsers(): Promise<LineUser[]> {
  ...
  try {
    ...
    return ((data ?? []) as LineUserRow[]).map((r) => ({
...
export async function getConversationHistory(
  ...
  try {
    ...
    return ((data ?? []) as ConversationLogRow[]).map((r) => ({
      id: r.id,
      lineUserId: r.line_user_id,
      role: r.role,
      content: r.content,
...
export async function resolveIssue(logId: string, resolved: boolean): Promise<boolean> {
  ...
  try {
    const { error } = await sb
      .rpc('jsonb_merge_metadata', { log_id: logId, patch: { resolved } })
      .maybeSingle();
    if (error) {
      const { data } = await sb
        .from('conversation_logs')
      ...
      const metadata = { ...existing, resolved };
...
export async function getCachedSummary(
  ...
  try {
    const { data } = await sb
      ...
      .eq('line_user_id', lineUserId)
      .single();
    const row = data as Pick<LineUserRow, 'summary' | 'summary_updated_at'> | null;
    if (row?.summary && row?.summary_updated_at) {
      return { summary: row.summary, updatedAt: row.summary_updated_at };
    }
    return null;
  } catch {
    return null;
...
export async function getConversationStats(): Promise<ConversationStats> {
  ...
  const cached = cache.stats as CacheEntry<ConversationStats> | null;
  if (cached && Date.now() - cached.timestamp < STATS_CACHE_TTL) return cached.data;
  ...
  try {
    ...
    const typedLogs = logs as ConversationLogRow[];
    const now = new Date();
    let totalApiCalls = 0,
      flagged = 0,
      totalLatency = 0,
    ...
    for (const log of typedLogs) {
      ...
      if (bucket) {
        ...
        if (meta.flagged) bucket.flagged++;
      }
    }
    ...
    const result: ConversationStats = {
      ...
      totalMessages: totalMessages || typedLogs.length,
      totalApiCalls,
    ...
    const err = e as { code?: string };
    if (err?.code !== '42P01') console.error('getConversationStats error:', e);
    return empty;
  }
}
...
export async function getCustomersWithContext(): Promise<CustomerWithContext[]> {
  ...
  try {
    ...
    const typedUsers = users as LineUserRow[];
    const typedLogs = (logs ?? []) as ConversationLogRow[];
    const typedReservations = (reservations ?? []) as unknown as ReservationWithPickupRow[];
    ...
    for (const log of typedLogs) {
      ...
      const isComplaint =
        log.role === 'user' &&
        !isFlagged &&
        COMPLAINT_KEYWORDS.some((kw) => (log.content || '').includes(kw));
    ...
    const pickupMap = new Map<
      string,
      { date: string; orderNumber: string | null; status: string }
    >();
    for (const r of typedReservations) {
      const date = r.pickup_availability?.available_date;
    ...
    const customers: CustomerWithContext[] = typedUsers.map((u) => {
    ...
    const err = e as { code?: string };
    if (err?.code !== '42P01') console.error('getCustomersWithContext error:', e);
    return [];
  }
}
