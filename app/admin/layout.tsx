...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [authState, setAuthState] = useState<{ authenticated: boolean; loading: boolean }>({
    authenticated: false,
    loading: true,
  });
  ...
  useEffect(() => {
    ...
    // Auth state derived from token check — intentional sync setState on mount
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setAuthState({
      authenticated: !!token || pathname === '/admin/login',
      loading: false,
    });
  }, [pathname, router]);
            ...
            return (
              <Link
                ...
                className={`flex-1 flex flex-col items-center justify-center gap-[3px] transition-colors ${
                  isActive ? 'text-amber-800' : 'text-stone-350 hover:text-stone-500 text-stone-400'
                }`}
              >
                {item.icon}
                <span
