...
export default function UsersPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([fetchCustomers(), fetchStats()]).then(() => {
  ...
  // Memoize current time for timeAgo to avoid react-hooks/purity rule
  ...
  const filteredCustomers = useMemo(() => {
    ...
    if (customerFilter === 'pickup') {
      return list
        .filter((c) => c.upcomingPickup)
        .sort((a, b) => (a.upcomingPickup || '').localeCompare(b.upcomingPickup || ''));
    }
  ...
  const flaggedTabCount = useMemo(
    () => customers.filter((c) => c.flaggedCount > 0).length,
    [customers],
  );
  ...
  if (selectedUser) {
    ...
    for (let i = 0; i < chronological.length; i++) {
      ...
      if (log.metadata?.flagged && !seenIds.has(log.id)) {
        let ctx = '';
        for (let j = i - 1; j >= 0; j--) {
          if (chronological[j].role === 'bot') {
            ctx = chronological[j].content;
            break;
          }
        }
        allIssues.push({
    ...
    async function toggleResolved(logId: string, resolved: boolean) {
      try {
        ...
        setHistory((prev) =>
          prev.map((l) => (l.id === logId ? { ...l, metadata: { ...l.metadata, resolved } } : l)),
        );
      } catch {
        toast('更新狀態失敗', 'error');
      }
      ...
      <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            {selectedUser.pictureUrl ? (
              <Image
                ...
                alt={selectedUser.displayName + ' 的大頭貼'}
                width={44}
                height={44}
                className="w-11 h-11 rounded-full"
              />
            ) : (
              <div className="w-11 h-11 rounded-full bg-stone-200 flex items-center justify-center text-stone-400">
                👤
              </div>
            )}
        ...
        {openIssues.length > 0 && (
          <div className="bg-white rounded-2xl border border-red-100 p-4">
            <p className="text-[10px] font-semibold text-red-500 mb-2.5">
              待處理（{openIssues.length}）
            </p>
            <div className="space-y-2">
              {openIssues.map((issue) => (
                <div key={issue.id} className="flex items-start gap-2.5">
                  <button
                    onClick={() => void toggleResolved(issue.id, true)}
            ...
            <div className="px-4 pb-3 space-y-2">
              {resolvedIssues.map((issue) => (
                <div key={issue.id} className="flex items-start gap-2.5 opacity-60">
                  <button
                    onClick={() => void toggleResolved(issue.id, false)}
  ...
  const chartValues = chartData.map((d) =>
    activeChart === 'apiCalls' ? d.apiCalls : activeChart === 'latency' ? d.avgLatency : d.tokens,
  );
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-2.5">
          {[
            {
              label: 'API 呼叫',
              value: String(stats.totalApiCalls),
              sub: `今日 ${chartData[chartData.length - 1]?.apiCalls || 0} 次`,
            ...
            },
            {
              ...
              value:
                stats.totalApiCalls > 0
                  ? `${((stats.flaggedCount / stats.totalApiCalls) * 100).toFixed(1)}%`
                  : '0%',
              sub: `${stats.flaggedCount} 則回報`,
              color: stats.flaggedCount > 0 ? 'text-red-600' : 'text-stone-800',
            },
          ].map((card) => (
            <div key={card.label} className="bg-white rounded-2xl border border-stone-100 p-3.5">
              ...
              <p className={`text-[22px] font-semibold leading-tight mt-0.5 ${card.color}`}>
                {card.value}
              </p>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-4">
          ...
          <div className="flex items-end gap-1.5" style={{ height: '96px' }}>
            {chartData.map((d, i) => {
              ...
              const barH =
                maxChartVal > 0 ? Math.max((val / maxChartVal) * 72, val > 0 ? 4 : 0) : 0;
              const label =
                activeChart === 'latency'
                  ? val > 0
        ...
        {filteredCustomers.length === 0 ? (
          <div className="bg-white rounded-2xl border border-stone-100 py-12 text-center">
            <p className="text-[13px] font-medium text-stone-600 mb-1">
              {customerFilter === 'pickup'
                ? '目前沒有近期取貨的顧客'
                : customerFilter === 'flagged'
                  ? '沒有問題回饋紀錄'
                  : '還沒有顧客紀錄'}
            </p>
          </div>
              ...
              >
                <div className="relative shrink-0">
                  {c.pictureUrl ? (
                    <Image
                      src={c.pictureUrl}
                      alt={c.displayName + ' 的大頭貼'}
                      width={36}
                      height={36}
                      className="w-9 h-9 rounded-full"
                    />
                  ) : (
                    <div className="w-9 h-9 rounded-full bg-stone-200 flex items-center justify-center text-stone-400 text-[11px]">
                      👤
                    </div>
                  )}
                  {c.flaggedCount > 0 && (
