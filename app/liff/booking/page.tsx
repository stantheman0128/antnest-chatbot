...
import type { LiffProfile, LiffState } from "@/lib/liff";
...
export default function LiffBookingPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setLiffState({ status: "error", error: e?.message || "LIFF 初始化失敗" });
      }
    })();
  }, []);
  ...
  async function handleSubmit() {
    if (liffState.status !== "ready" || !selected || !form.displayName.trim() || !form.pickupTime) return;
    setSubmitting(true);
    setError(null);
  ...
  async function handleSaveEdit(reservationId: string) {
    ...
    try {
      ...
      await fetch("/api/liff/reservations", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reservationId, lineUserId: userId, action: "update_note", value: editForm.note }),
      });
      // Update order number
      await fetch("/api/liff/reservations", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reservationId, lineUserId: userId, action: "update_order", value: editForm.orderNumber }),
      });
      setReservations((prev) =>
        prev.map((r) =>
          r.id === reservationId ? { ...r, note: editForm.note || null, orderNumber: editForm.orderNumber || null } : r
        )
      );
      setEditingId(null);
    } catch { /* ignore */ }
      ...
      <div className="min-h-screen bg-amber-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-sm">
          ...
          <p className="text-amber-700 mb-4">
            {formatDateLabel(selected.availableDate)}<br />
          ...
          <div className="flex gap-3">
            <button
              onClick={() => { setBookingStep("select-date"); setSelected(null); setForm((f) => ({ ...f, pickupTime: "", orderNumber: "", note: "" })); }}
            ...
            <button
              onClick={() => { setTab("my"); fetchMyReservations(); }}
    ...
    <div className="min-h-screen bg-amber-50">
      ...
      <div className="px-4 pt-3">
        <div className="flex bg-white rounded-xl border border-stone-200 p-1 gap-1">
          {([["book", "預約取貨"], ["my", "我的預約"]] as const).map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`flex-1 py-2 text-[13px] font-medium rounded-lg transition-colors ${
                tab === key ? "bg-amber-800 text-white shadow-sm" : "text-stone-500 hover:text-stone-700"
              }`}
            >
              {label}
            </button>
        ...
        <div className="p-4">
          {bookingStep === "select-date" && (
            <div className="max-w-sm mx-auto">
              ...
              {error && (
                <div className="bg-red-50 text-red-700 rounded-xl p-3 text-[13px] mb-4">{error}</div>
              )}
              ...
              {loadingSlots ? (
                ...
                <div className="space-y-3">
                  {availabilities.map((avail) => (
                    ...
                    >
                      <div>
                        <p className="text-[13px] font-bold text-amber-900">{formatDateLabel(avail.availableDate)}</p>
                        <p className="text-[11px] text-amber-500 mt-0.5">
                          取貨時段：{avail.startTime.slice(0, 5)}–{avail.endTime.slice(0, 5)}
                          <span className="ml-2 text-stone-400">剩 {avail.maxBookings - avail.currentBookings} 位</span>
                        </p>
                      </div>
          ...
          {bookingStep === "fill-form" && selected && (
            <div className="max-w-sm mx-auto">
              ...
              <div className="bg-amber-100 rounded-xl p-4 mb-6">
                ...
                <p className="text-[13px] font-bold text-amber-900">{formatDateLabel(selected.availableDate)}</p>
                <p className="text-[11px] text-amber-600">可取貨時間：{selected.startTime.slice(0, 5)}–{selected.endTime.slice(0, 5)}</p>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[11px] font-medium text-amber-800 block mb-1">取貨時間 *</label>
                  <input
                    type="time"
                  ...
                  />
                  <p className="text-[10px] text-amber-500 mt-1">請選擇 {selected.startTime.slice(0, 5)}–{selected.endTime.slice(0, 5)} 之間的時間</p>
                </div>

                <div>
                  <label className="text-[11px] font-medium text-amber-800 block mb-1">姓名 *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[11px] font-medium text-amber-800 block mb-1">訂單編號（選填）</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[11px] font-medium text-amber-800 block mb-1">備註（選填）</label>
                  <textarea
                    value={form.note}
                ...
                <div key={r.id} className="bg-white rounded-xl border border-stone-100 overflow-hidden">
                  <div className="px-4 py-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${STATUS_STYLE[r.status] || "bg-stone-100 text-stone-500"}`}>
                        {STATUS_LABEL[r.status] || r.status}
                      </span>
                    </div>
                    <p className="text-[11px] text-stone-500">取貨時間：{r.pickupTime?.slice(0, 5)}</p>
                    {r.orderNumber && <p className="text-[11px] text-stone-400 mt-0.5">訂單：{r.orderNumber}</p>}
                    {r.note && <p className="text-[11px] text-stone-400 mt-0.5 italic">備註：{r.note}</p>}

                    {/* Edit form */}
                    {editingId === r.id && (
                      <div className="mt-3 space-y-2 bg-stone-50 rounded-lg p-3">
                        <div>
                          <label className="text-[10px] font-medium text-stone-500 block mb-1">訂單編號</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="text-[10px] font-medium text-stone-500 block mb-1">備註</label>
                          <textarea
                            value={editForm.note}
