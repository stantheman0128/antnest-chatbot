...
const handles = [...new Set(
  [...xml.matchAll(/\/products\/([^<\s"']+)/g)].map(m => m[1])
)].filter(h => !h.includes(".") && h.length > 2); // exclude .xml, .atom, etc
...
for (const handle of handles) {
  ...
  if (!maximum) console.log(`   photo_urls[0] keys: ${photo ? Object.keys(photo).join(", ") : "(none)"}`);
}
...
if (fail > 0) {
  ...
  results.filter(r => !r.ok).forEach(r => console.log(`   ${r.handle}: ${r.reason ?? "no maximum key"}`));
}
