...
if (!htmlRes.ok) { console.error(`HTML HTTP ${htmlRes.status}`); process.exit(1); }
...
const matchB = html.match(/<meta[^>]+property="og:image"[^>]+content="([^"]+)"/)
            || html.match(/<meta[^>]+content="([^"]+)"[^>]+property="og:image"/);
const urlB = matchB?.[1] ?? null;

// ─── Method C: cdn 600x600+ (擴大 regex) ─────────────────────────────────────
const matchC = html.match(/https:\/\/cdn(?:-next)?\.cybassets\.com\/media\/[^"'\s]+(?:2048x2048|1200x1200|600x600)[^"'\s]*/);
...
let urlLD = null;
try { urlLD = JSON.parse(ldRaw?.[1] ?? "null")?.image ?? null; } catch {}
