...
async function seedProducts() {
  ...
  const cards = JSON.parse(fs.readFileSync(cardsPath, 'utf-8')) as Record<string, SeedCard>;
  ...
  const products: ProductInsert[] = Object.entries(cards).map(([id, card], i) => {
    // Extract detailed description from system prompt
    const regex = new RegExp(`<product id="${id}">[\\s\\S]*?</product>`, 'i');
    const match = prompt.match(regex);
    const detailedDesc = match
      ? match[0].replace(`<product id="${id}">`, '').replace('</product>', '').trim()
      : null;
  ...
  const spoonMatch = prompt.match(/<product id="spoon">[\s\S]*?<\/product>/i);
  if (spoonMatch) {
    products.push({
      id: 'spoon',
      name: '鏟子湯匙（加購專區）',
...
async function seedConfig() {
  ...
  const rulesMatch = content.match(/<rules priority="由高到低">([\s\S]*?)<\/rules>/i);
  ...
  const { error } = await supabase.from('system_config').upsert(configs, { onConflict: 'key' });
